<x-dashboard>
    ...
    @if (session('success'))
        <div id="popupAlert"
            class="fixed top-4 left-1/2 transform -translate-x-1/2 bg-green-200 px-6 py-4 rounded-md text-lg flex items-center mx-auto max-w-lg z-50">
    ...
    @if (session('error'))
        <div id="popupAlert"
            class="fixed top-4 left-1/2 transform -translate-x-1/2 bg-red-200 px-6 py-4 rounded-md text-lg flex items-center mx-auto max-w-lg z-50">
    ...
    @if (session('warning'))
        <div id="popupAlert"
            class="fixed top-4 left-1/2 transform -translate-x-1/2 bg-orange-200 px-6 py-4 rounded-md text-lg flex items-center mx-auto max-w-lg z-50">
    ...
    <div class="flex flex-col flex-grow overflow-y-auto">
        <div class="p-8">
            <h1 class="text-2xl font-bold">Hasil Inputan User</h1>
                    ...
                    <div class="min-w-full inline-block align-middle">
                        <div class="relative  text-gray-500 focus-within:text-gray-900 mb-4">
                            <div class="flex gap-4 items-center">
                                <form action="{{ route('moveToPersonAttributeAll') }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="px-6 py-4 text-white bg-gray-800 hover:bg-gray-700 rounded-md shadow-md"
                                        onclick="return confirm('Apakah Anda yakin ingin memindahkan semua data ke dataset?');">
                                        Masukkan data ke dataset
                                    </button>
                                </form>
                                <form id="delete-all-form" action="{{ route('user-input.deleteAll') }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                        class="inline-block w-auto text-center min-w-[200px] px-6 py-4 text-white transition-all bg-red-700 rounded-md shadow-xl sm:w-auto hover:bg-red-900 hover:text-white"
                                ...
                                <thead>
                                    <tr class="bg-gray-50">
                                        <th scope="col"
                                            ...
                                            No</th>
                                        <th scope="col"
                                            class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize border-b px-4 py-2">
                                            Gender</th>
                                        <th scope="col"
                                            class="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize border-b px-4 py-2">
                                            Age (Years)</th>
                                ...
                                <tbody class="divide-y divide-gray-300">
                                    @foreach ($userinputs as $userinput)
                                        <tr class="bg-white transition-all duration-500 hover:bg-gray-50">
                                            ...
                                            <td
                                                class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                {{ number_format($userinput->height, 0) }} Cm
                                            </td>
                                            <td
                                                class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                {{ number_format($userinput->weight, 0) }} Kg
                                            </td>
                                            <td
                                                class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                {{ $userinput->familyhistoryLabel }}
                                            </td>
                                            <td
                                                class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                {{ $userinput->favcLabel }}
                                            ...
                                            <td
                                                class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                {{ $userinput->mtransLabel }}
                                            </td>
                                            <td
                                                class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                                                {{ $userinput->nobeyesdad }}
                                            </td>
                                            <td class="p-5">
                        ...
                        @include('pagination', ['userinputs' => $userinputs])
    ...
    <!-- Script JavaScript diletakkan di bagian bawah -->
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (popupAlert) {
                setTimeout(function() {
                    ...
                    setTimeout(() => popupAlert.remove(), 500); // Remove the alert after fade-out
